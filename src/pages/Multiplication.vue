...
<script>
  ...
  const CONFIG = {
    APP: '', // empty is for multiplication for consistency
    PATH: 'multiplication', // empty is for multiplication for consistency
    UNDEFINED: '…',
    BONUS_TIME: 6000,
    MAX_TIME: 15000,
    COOLDOWN: 1000,
  ...
  export default {
    ...
    mounted () {
      ...
      common.readSessionDataThenGenerateTask(vm)

      window.scrollTo(0, document.body.scrollHeight)

      window.addEventListener('keyup', function (event) {
        // console.log(event)
        common.valueChange(vm, event.key)
      })
    }
  }
